...
class TrajectoryCalculator:
    ...
    def get_motor_params(self,J, T_e, Umax, Fi, Ce, Ra, Cm):
        # self.J = J
        # self.n = n
        # self.Umax = Umax
        # self.Ku = Ku
    ...
    def ROBOT_function(self, q1, q2, t):
        ...
        for i in range(len(t)):
            ...
            for j in steps:
                ...
                # Звено 1
                if Ra[0] != 0:
                    nI_Ra_1 = U_changed_1 / Ra[0]
                else:
                    nI_Ra_1 = 0

                I_a_1 = I_a_1 + (nI_Ra_1 * (1 / T_I[0]) - I_a_1 * (1 / T_I[0])) * (accuracy)
                I_a__Cm_1 = I_a_1 * Cm[0]

                # Звено 2
                if Ra[1] != 0:
                    nI_Ra_2 = U_changed_2 / Ra[1]
                else:
                    nI_Ra_2 = 0

                I_a_2 = I_a_2 + (nI_Ra_2 * (1 / T_I[1]) - I_a_2 * (1 / T_I[1])) * (accuracy)
...
class RobotTrajectoryApp(QMainWindow):
    ...
    def show_creators_info(self):
        ...
        layout = QVBoxLayout()
        creators_label = QLabel("<b>Создатели ")
        creators_label.setFont(QFont('Times', 20))
        ...
        names_label = QLabel(
            ...
            "Backend-разработчик: Охрименко Ф.И.\n"
            "Главный по github и гороскопам: Багаева М.Н.\n"
            "Дизайнер: Болотов М.А."
        )
        names_label.setFont(QFont('Times', 16))
        names_label.setAlignment(Qt.AlignCenter)
        layout.addWidget(names_label)

        # budget_label = QLabel("<b>Бюджет разработки")
        # budget_label.setFont(QFont('Times', 14))
    ...
    def show_motor_params_dialog(self):
        ...
        for row in range(7):
            for col in range(2):
                ...
                elif key.startswith("T_e"):
                    default_value = self.T_e[col]
                elif key.startswith("Umax"):
                    default_value = self.Umax[col]
                elif key.startswith("Fi"):
                    default_value = self.Fi[col]
                elif key.startswith("Ce"):
                    default_value = self.Ce[col]
                elif key.startswith("Ra"):
                    default_value = self.Ra[col]
                elif key.startswith("Cm"):
                    default_value = self.Cm[col]
    ...
    def show_motor_params_help_dialog(self):
        ...
        help_text.setText("""К числу параметров двигателя относятся:  <br>
                            ...
                            - T_e — электрическая постоянная; <br>
                            - Umax — максимальное напряжение двигателя; <br>
                            - Fi — магнитный поток; <br>
                            - Ce — коэффициент электрический; <br>
    ...
    def apply_motor_params(self, dialog):
        try:
            ...
            for key, input_field in self.motor_inputs.items():
                ...
                if key.startswith("J") and not (0 <= value <= 10):
                    self.show_error(f"Значение {key} не в диапазоне от 0 до 1")
                    return

                # Проверка для T_e1 T_e4 (диапазон 0-500)
                if key.startswith("T_e") and not (0 <= value <= 10):
                    self.show_error(f"Значение {key} не в диапазоне от 0 до 10")
                    return
                ...
                if key.startswith("U") and "max" in key and not (0 <= value <= 40):
                    ...
                    return
                # Проверка для Fi1 Fi4 (диапазон 0-500)
                if key.startswith("Fi") and not (0 <= value <= 10):
                    self.show_error(f"Значение {key} не в диапазоне от 0 до 10")
                    return
                # Проверка для Ce1 Ce4 (диапазон 0-500)
                if key.startswith("Ce") and not (0 <= value <= 10):
                    self.show_error(f"Значение {key} не в диапазоне от 0 до 10")
                    return
                # Проверка для Ra1 Ra4 (диапазон 0-500)
                if key.startswith("Ra") and not (0 <= value <= 10):
                    self.show_error(f"Значение {key} не в диапазоне от 0 до 10")
                    return
                # Проверка для Cm1 Cm4 (диапазон 0-500)
                if key.startswith("Cm") and not (0 <= value <= 10):
                    self.show_error(f"Значение {key} не в диапазоне от 0 до 10")
                    return
            ...
            self.trajectory_calculator.get_motor_params(self.J, self.T_e, self.Umax, self.Fi, self.Ce, self.Ra, self.Cm)
            dialog.accept()
        ...
        if file_name:
            try:
                with open(file_name, 'r') as file:
                    ...
                    self.T_e = list(map(float, lines[62].strip().split(',')))
                    self.Umax = list(map(float, lines[63].strip().split(',')))
                    self.Fi = list(map(float, lines[64].strip().split(',')))
                    self.Ce = list(map(float, lines[65].strip().split(',')))
                    self.Ra = list(map(float, lines[66].strip().split(',')))
                    self.Cm = list(map(float, lines[67].strip().split(',')))

                    self.Kp = list(map(float, lines[66+2].strip().split(',')))
                    self.Ki = list(map(float, lines[67+2].strip().split(',')))
                    self.Kd = list(map(float, lines[68+2].strip().split(',')))

                    self.t = list(map(float, lines[69+2].strip().split(',')))
                    self.q1 = list(map(float, lines[70+2].strip().split(',')))
                    self.q2 = list(map(float, lines[71+2].strip().split(',')))
                    self.q3 = list(map(float, lines[72+2].strip().split(',')))
                    self.q4 = list(map(float, lines[73+2].strip().split(',')))

                    self.trajectory_type = lines[74+2].strip()
                    ...
                    self.line_params = list(map(float, lines[75+2].strip().split(',')))
                    self.circle_params = list(map(float, lines[76+2].strip().split(',')))
        ...
        if self.current_file_path:
            try:
                with open(self.current_file_path, 'w') as file:
                    ...
                    file.write(','.join(map(str, self.T_e)) + '\n')
                    file.write(','.join(map(str, self.Umax)) + '\n')
                    file.write(','.join(map(str, self.Fi)) + '\n')
                    file.write(','.join(map(str, self.Ce)) + '\n')
                    file.write(','.join(map(str, self.Ra)) + '\n')
                    file.write(','.join(map(str, self.Cm)) + '\n')
